class HanoiGame:
    def __init__(self):
        self.poles = [[3, 2, 1], [], []]  # начальное состояние: все кольца на первом столбе
        self.moves = 0  # счётчик ходов
        self.selected_pole = None  # выбранный исходный столб
        self.target_pole = None  # выбранный целевой столб

    def select_pole(self, pole_index):
        """Выбор столба (исходного или целевого)"""
    ...
    def move_ring(self):
        """Перемещение кольца с выбранного столба на целевой"""
        if self.selected_pole is None:
            return False, "Сначала выберите исходный столб!"
        ...
        ring = self.poles[self.selected_pole][-1]  # верхнее кольцо
        if self.poles[self.target_pole] and self.poles[self.target_pole][-1] < ring:
    ...
    def is_won(self):
        """Проверка на победу: все кольца на третьем столбе"""
